...
int AOpenCVActor::GetColorMode_CV() {
...
void AOpenCVActor::OnBackBufferReady(SWindow& SlateWindow, const FTexture2DRHIRef& BackBuffer) {
...
void AOpenCVActor::NextCamera()
{
    CameraID += 1;
    ValidateCameraID();
}
//increment video track by one then validate exists
void AOpenCVActor::NextVideoTrack()
{
    VideoTrackID += 1;
...
void AOpenCVActor::EnablePlayerMode() {
    InputMode = EInputMode::Player;
    //OnBackBufferReadyToPresent = FSlateApplication::Get().GetRenderer()->OnBackBufferReadyToPresent().AddUObject(this, &AOpenCVActor::OnBackBufferReady);
}
// unbind delegate to disable player mode
void AOpenCVActor::DisablePlayerMode() {
    OnBackBufferReadyToPresent.Reset();
}

void AOpenCVActor::EnableCameraMode() {
    InputMode = EInputMode::Camera;
}

void AOpenCVActor::DisableCameraMode() {

}

void AOpenCVActor::EnableSceneMode() {
    InputMode = EInputMode::Scene;
}

void AOpenCVActor::DisableSceneMode() {
